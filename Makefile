...
LINGUIST_ECR_URI=$(shell AWS_PROFILE=${AWS_PROFILE} AWS_REGION=${AWS_REGION} aws ecr describe-repositories --repository-names ml/${APP} | jq -r ".repositories[0].repositoryUri"):${VERSION_TAG}
AWS_PROFILE=welco
...
ifeq ($(OS), darwin)
	pip uninstall -y rsyslog_cee
	pip uninstall -y bandolier
	pip uninstall -y linguist
endif
ifeq  ($(OS), linux)
	$(linux_pip) uninstall -y rsyslog_cee
	$(linux_pip) uninstall -y bandolier
	$(linux_pip) uninstall -y linguist
endif
...
.PHONY : docker_linguist_requirements
docker_linguist_requirements: docker_python
	./docker_build_requirements.sh

.PHONY : docker_build
docker_build:
	./docker_build.sh
	docker tag linguist $(LINGUIST_ECR_URI)
...
.PHONY: lin-bash
lin-bash:
	docker exec -it linguist /bin/bash